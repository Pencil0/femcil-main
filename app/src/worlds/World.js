...
class World {
    ...
    resolveRigidCheck(a, b) {
        ...
        a.x -= (dx * m * aM)/3;   //bounce force (reccomended value is divided by 3)
        a.y -= (dy * m * aM)/3;
        b.x += (dx * m * bM)/3;
        b.y += (dy * m * bM)/3;
        this.bounceCell(a);
 ...
 /*     resolveRigidCheck(a, b) {
    ...
    /** @param {Cell} cell */
    boostCell(cell) {
        const d = cell.boost.d / 20 * this.handle.stepMult; // speed or velo of splits
    ...
    */
    ...
    launchPlayerCell(cell, size, boost) {
        ...
        const x = cell.x + 75 * boost.dx; //solotricks
        const y = cell.y + 75 * boost.dy;
